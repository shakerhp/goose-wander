create table if not exists public.goose_events (
  ...
  guest_name text,
  egg_count int4 check (egg_count between 1 and 5),
...
alter table public.goose_events add column if not exists egg_count int4;
...
alter table public.goose_events drop constraint if exists goose_events_egg_count_check;
alter table public.goose_events
  add constraint goose_events_egg_count_check check (egg_count is null or egg_count between 1 and 5);
